...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_weights", type=str, default='checkpoints_tiny', help="if specified starts from checkpoint model")
    ...
    os.makedirs(opt.pretrained_weights, exist_ok=True)
    ...
    if opt.pretrained_weights:
        chkps = os.listdir(opt.pretrained_weights)
        if len(chkps)>0:
            pretrained_weights = sorted(chkps)[-1]
            model.load_state_dict(torch.load(pretrained_weights))
            start_epoch = int(pretrained_weights.split('.').split('_')[-1])+1
